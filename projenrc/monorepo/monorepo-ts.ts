...
import { type Dependency, DependencyType, JsonFile, type Project, type Task, YamlFile } from 'projen'
import type { JavaProject } from 'projen/lib/java'
import { NodePackageManager, NodeProject } from 'projen/lib/javascript'
import type { PythonProject } from 'projen/lib/python'
import { TypeScriptProject, type TypeScriptProjectOptions } from 'projen/lib/typescript'
import { DEFAULT_PROJEN_VERSION, syncProjenVersions } from './projen-dependency'
 ...
 * @pjid monorepo-ts
 */
export class MonorepoTsProject extends TypeScriptProject implements INxProjectCore {
  /**
  ...
  constructor(options: MonorepoTsProjectOptions) {
    ...
    super({
      ...
      projenCommand: options.packageManager ? NodePackageUtils.command.projen(options.packageManager) : undefined,
    ...
    this.package.setScript(
      'install:ci',
      !this.ejected
        ? NodePackageUtils.command.exec(this.package.packageManager, 'projen install:ci')
    ...
    switch (this.package.packageManager) {
      ...
      case NodePackageManager.YARN2: {
        ...
        this.package.setScript(
          'install:ci',
          !this.ejected
            ? NodePackageUtils.command.exec(NodePackageManager.NPM, 'projen install:ci')
            : 'scripts/run-task install:ci',
        )
        this.package.setScript(
          'install',
          !this.ejected
            ? NodePackageUtils.command.exec(NodePackageManager.NPM, 'projen install')
    ...
    !this.ejected &&
      this.package.setScript('synth-workspace', NodePackageUtils.command.projen(this.package.packageManager))
  ...
  public addWorkspacePackages(...packageGlobs: string[]) {
    ...
    this.workspacePackages.push(
      ...relativeSubProjectWorkspacePackages.filter((pkg) => !existingWorkspacePackages.has(pkg)),
    )
  ...
  public get sortedSubProjects(): Project[] {
    return this.subprojects.slice().sort((a, b) => a.name.localeCompare(b.name))
  }
  ...
  protected linkLocalWorkspaceBins(): void {
    ...
    this.subprojects.forEach((subProject) => {
      if (ProjectUtils.isNamedInstanceOf(subProject, NodeProject) && subProject.name !== '@aws/pdk') {
        const pkgBins: Record<string, string> = subProject.package.manifest.bin() || {}
        bins.push(
    ...
    const linkTask = this.addTask('workspace:bin:link', {
      steps: bins.map(([cmd, bin]) => ({
        exec: `ln -s ${bin} ${NodePackageUtils.command.bin(this.package.packageManager, cmd)} &>/dev/null; exit 0;`,
      })),
    })
  ...
  private addUpgradeDepsTask() {
    if (this._options.monorepoUpgradeDeps !== false) {
      ...
      const nonNodeSubprojects = this.subprojects.filter((p) => !NodePackageUtils.isNodeProject(p))
      ...
      upgradeDepsTask.exec(NodePackageUtils.command.exec(this.package.packageManager, 'syncpack', 'fix-mismatches'))
      upgradeDepsTask.exec(`rm ${this.package.lockFile}`)
      upgradeDepsTask.exec(NodePackageUtils.command.install(this.package.packageManager))
      !this.ejected && upgradeDepsTask.exec(NodePackageUtils.command.exec(this.package.packageManager, 'projen'))

      new JsonFile(this, '.syncpackrc.json', {
        obj: this._options.monorepoUpgradeDepsOptions?.syncpackConfig || Syncpack.DEFAULT_CONFIG,
        readonly: true,
      })
    }
  }
    ...
    if (
      ...
      const noHoistBundled = this.subprojects.flatMap((sub) => {
        if (ProjectUtils.isNamedInstanceOf(sub, NodeProject)) {
          return getBundledDeps(sub).flatMap((dep) => [`${sub.name}/${dep.name}`, `${sub.name}/${dep.name}/*`])
        }
        return []
      })
  ...
  private installNonNodeDependencies() {
    const installProjects = this.subprojects.filter(
      (project) => !ProjectUtils.isNamedInstanceOf(project, NodeProject) && project.tasks.tryFind('install'),
    )
    ...
    if (installProjects.length > 0) {
      ...
      postinstallTask.exec(NodePackageUtils.command.exec(this.package.packageManager, ...nxRunManyInstall))
